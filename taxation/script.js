...
var taxationRate = 0.30; // redistribute....
/* don't put anything before the call to 'onStart()' exception GLOBALS (and have no more than 10 o those!). */
/* on start... */
function onStart() {
    startGame(false);
}
function startGame(startLoop) {
    initGame(populationSize, betPercent, frameDelay, startLoop, moneyPerPlayer, tradeValueFactor, taxationRate);
}
...
var Game = /** @class */ (function () {
    function Game(populationSize, maxBetPercent, frameDelay, moneyPerPlayer, tradeValueFactor, taxationRate) {
        this.Paused = true;
        this.FrameDelay = frameDelay !== null && frameDelay !== void 0 ? frameDelay : 100;
        this.NextId = 0;
        this.Rounds = 0;
        this.MaxPopulationSize = populationSize !== null && populationSize !== void 0 ? populationSize : 100;
        this.MaxBetPercent = maxBetPercent !== null && maxBetPercent !== void 0 ? maxBetPercent : 20;
        this.People = [];
        ...
        this.TaxationRate = taxationRate !== null && taxationRate !== void 0 ? taxationRate : 0.12;
        this.TaxCollected = 0.0;
        this.MedianWealth = moneyPerPlayer;
    ...
    Game.prototype.Tick = function () {
        ...
        for (var index in this.People) {
            ...
            this.Trade(this.People[index], this.People[(+index + numPairs)], this.MaxBetPercent, this.TradeValueFactor);
        }
        this.CalculateGini();
        this.DistributeTaxes();
        //this.ApplyTaxation();
        this.Rounds += 1;
    };
    ...
    Game.prototype.Trade = function (a, b, maxBetPercent, tradeValueFactor) {
        ...
        if (winner == a) {
            ...
            b.money -= maxBet;
            b.losses += 1;
            if (a.money > this.MedianWealth) {
                a.money -= maxBet * this.TaxationRate;
                this.TaxCollected += maxBet * this.TaxationRate;
            }
        }
        else {
            //winner == b
            ...
            a.money -= maxBet;
            a.losses += 1;
            if (b.money > this.MedianWealth) {
                b.money -= maxBet * this.TaxationRate;
                this.TaxCollected += maxBet * this.TaxationRate;
            }
        }
        if (a.luck != b.luck) {
    ...
    Game.prototype.DistributeTaxes = function () {
        if (this.TaxCollected <= 0 || this.People.length == 0) {
            return;
        }
        var distribution = this.TaxCollected / this.People.length;
        // okay - this is just universal income.
        for (var _i = 0, _a = this.People; _i < _a.length; _i++) {
            var person = _a[_i];
            person.money += distribution;
        }
    };
    Game.prototype.CalculateMedian = function () {
        // sort by wealth ascending
        this.People.sort(function (a, b) { return a.money - b.money; });
        var peopleCount = this.People.length;
        var medianPersonIndex = Math.round(peopleCount / 2);
        var medianPerson = this.People[medianPersonIndex];
        this.MedianWealth = medianPerson.money;
        return this.MedianWealth;
    };
    Game.prototype.CalculateGini = function () {
        // sort by wealth ascending
        this.People.sort(function (a, b) { return a.money - b.money; });
        var wealthTotal = 0;
        var peopleCount = this.People.length;
        ...
        var cumulativeFractionOfIncome = 0;
        var i = 0;
        for (var _b = 0, _c = this.People; _b < _c.length; _b++) {
            var person = _c[_b];
            i++;
            var fractionOfPopulation = 1 / peopleCount;
...
var TheGame;
function initGame(populationSize, maxBetPercent, frameDelay, startLoop, moneyPerPlayer, tradeValueFactor, taxationRate) {
    TheGame = new Game(populationSize, maxBetPercent, frameDelay, moneyPerPlayer, tradeValueFactor, taxationRate);
...
// function resetGame(populationSize: number, maxBetPercent: number, frameDelay:number) {
//     TheGame = new Game(populationSize, maxBetPercent);
//     initGameScreen(TheGame);
...
function updateGameScreen(game) {
    ...
    if (board != null) {
        ...
        Array.from(board.querySelectorAll('.person'))
            ...
            .forEach(function (person) { return board.appendChild(person); });
        var allPlayers = board.querySelectorAll('.person');
        var poorest = allPlayers.item(0);
        var richest = allPlayers.item(allPlayers.length - 1);
        var minValue = parseFloat((_a = poorest.getAttribute("data-money")) !== null && _a !== void 0 ? _a : "0");
        var maxValue = parseFloat((_b = richest.getAttribute("data-money")) !== null && _b !== void 0 ? _b : "0");
        game.RichestPersonMoney = maxValue;
        for (var index in game.People) {
            var person = game.People[index];
            updatePersonPanel(person, maxValue);
        }
    }
    // And now sort them....
    //let people =
...
function faster() {
    if (TheGame == null)
        return;
    if (TheGame.FrameDelay > 0) {
        TheGame.FrameDelay = Math.max(1, TheGame.FrameDelay - 5);
...
function slower() {
    if (TheGame == null)
        return;
    TheGame.FrameDelay = TheGame.FrameDelay + 5;
    updateGameSummary(TheGame);
}
function muchFaster() {
    if (TheGame == null)
        return;
    if (TheGame.FrameDelay > 0) {
        TheGame.FrameDelay = Math.max(1, TheGame.FrameDelay - 150);
        updateGameSummary(TheGame);
    }
}
function muchSlower() {
