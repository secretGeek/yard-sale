...
let taxationRate = 0.30; // redistribute....
...
class Person {
    ...
    id: string; // Person ID begins with a 'p'
    //skills: string[];
...
class Game {
    constructor(populationSize: number | null, maxBetPercent: number | null, frameDelay: number | null, moneyPerPlayer: number, tradeValueFactor: number, taxationRate: number) {
        this.Paused = true;
        ...
        this.TaxationRate = taxationRate ?? 0.12;
    ...
    DistributeTaxes() {
        ...
        let distribution = this.TaxCollected / this.People.length;
    ...
    CalculateMedian():number {
        ...
        let medianPersonIndex = Math.round(peopleCount / 2);
        let medianPerson:Person = this.People[medianPersonIndex];
    ...
    CalculateGini(): number {
        ...
        for (const person of this.People) {
            i++;
            let fractionOfPopulation = 1 / peopleCount;
...
function initGame(populationSize: number, maxBetPercent: number, frameDelay: number, startLoop: boolean, moneyPerPlayer: number, tradeValueFactor:number, taxationRate:number) {
...
function onLoop() {
    if (TheGame == null || TheGame.Paused) return;
...
function UpdateGame(game: Game): Game {
    game.Tick();
    return game;
}
...
function initGameScreen(game: Game) {
    let board = $id("board");
    ...
    for (const personId in game.People) {
        ...
        let personDiv = htmlToElement(`<div id='${game.People[personId].id}' class='person' data-money='${game.People[personId].money}'>${game.People[personId].name}</div>`);
...
function formatFloat(money:number):string {
    ...
    if (money < 0.001) return money.toFixed(5);
    if (money < 0.01) return money.toFixed(4);
    if (money < 0.1) return money.toFixed(3);
...
function updateGameScreen(game: Game) {
    ...
    if (board != null) {
        ...
        Array.from(board.querySelectorAll('.person'))
            .sort((a, b) => parseFloat(a.getAttribute("data-money") ?? "0") - parseFloat(b.getAttribute("data-money") ?? "0"))
            //.sort(comparer(Array.from(th.parentNode.children).indexOf(th), this.asc = !this.asc))
            .forEach(person => board.appendChild(person));

        let allPlayers = board.querySelectorAll('.person');
        let poorest = allPlayers.item(0);
        let richest = allPlayers.item(allPlayers.length - 1);

        let minValue = parseFloat(poorest.getAttribute("data-money") ?? "0")
        let maxValue = parseFloat(richest.getAttribute("data-money") ?? "0")
...
function startStopGame(btn: HTMLElement) {
    if (TheGame == null) return;
